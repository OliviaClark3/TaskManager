...
const DisplayTodos = (props) => {
    ...
    const handleChecked = async(index, checked) => {
        ...
        let resortedTodos = sortTodos(newTodos);
        props.setPropsTodos(resortedTodos);
        props.setTodos(resortedTodos);
    }
                ...
                {props.todos.map((todoList, todoListIndex) => {
                    ...
                    if (todoListIndex == 0) {
                        return todoList.map((todo, index) => (
                            <>
                                {console.log('mapping ' + JSON.stringify(todo) + ' ' + todo[0].data.completed)}
